...
V54: App icon source is `Assets/Icon.icon` (Apple Icon Composer format, macOS 26+). `icon.json` MUST declare `fill-specializations` for `dark-color`, `blend-mode-specializations` for dark/clear, and `glass-specializations` for clear so the icon adapts to light, dark, and Liquid Glass (clear) appearances. Compiled by `xcrun actool` to `Icon.icns` + `Assets.car` (the .car carries all 3 appearance variants — `NSAppearanceNameAqua`, `NSAppearanceNameDarkAqua`, `ISAppearanceTintable`). Both files are bundled into `Sources/AerialWall/Resources/` for `swift run` use and into `AerialWall.app/Contents/Resources/` for proper bundle use. Note: the legacy `Icon.icns` only carries the base appearance; full appearance switching requires the .app bundle path (CFBundleIconName="Icon" + Assets.car) — `swift run` will show a static light variant only.
...
B19|2026-05-19|Desktop apply showed black until lock+unlock. Hypothesised "level 3 reader needs temporal_id ≥ 3, VT caps at 2 at 4K". Led to V50 (2880p, 4-layer). But user never re-imported when testing V50 (library still had old files). B20 A/B/C/D test disproved the hypothesis. |B20
B20|2026-05-19|A/B/C/D variant test using Apple Tahoe Day as source (removes user-content variables). A=4K-single, B=4K-2layer, C=2880p-4layer, D=2880p-single. A+D: unlock gray. B+C: all paths work including desktop apply. Proves: (1) temporal sub-layers are required; (2) 2 layers is sufficient; (3) level-3-needs-temporal_id-3 hypothesis was wrong; (4) 4K works fine with 2-layer. **B19's 2880p resolution downgrade was unnecessary.** Reverted to V48: 4K + `BaseLayerFrameRate=srcFps/2`. Root cause of earlier "apply shows black" for user content unknown — likely user tested stale pre-V48 files or there's a scaling-path issue to investigate when user re-imports.| —
```
