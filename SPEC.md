...
V47: default encoder = `libx265` (software). `hevc_videotoolbox` output causes `WallpaperAerialsExtension` unlock-rebind to fall back to gray (B17 — Test C definitive: even Apple's own content re-encoded through VT fails). VT-specific `pix_fmt p010le` swapped for libx265's `yuv420p10le`. x265 params: `bframes=4:ref=4:level-idc=5.2`. `-preset fast` keeps 4K Main 10 encodes tractable (~realtime). VT retained as explicit opt-in via `TranscodeOptions.encoder = .videoToolbox` for future research
...
B17|2026-05-18|Definitive Test C: re-encoded Apple's Tahoe Day through our exact pipeline (hevc_videotoolbox, -bf 4, setpts=0, etc.), injected as AerialWall, applied — STILL went gray on unlock. The .mov produced by our VT pipeline is unconditionally rejected by `WallpaperAerialsExtension` unlock-rebind, regardless of input. The specific VT property that triggers rejection is unidentified. Fix: V47 — switch default encoder to libx265 (software, canonical HEVC, full B-frame/level control). Slower (~realtime on 4K Main 10) but produces output WallpaperAerialsExtension accepts.|V47
```
