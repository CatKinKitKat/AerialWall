...
V45: transcoded `.mov` output framerate = `240/1` to match Apple's `4KSDR240FPS` directory convention. With 30fps source ffmpeg duplicates frames (~8× count, modest size increase since dupes encode cheaply). Lower framerates produce sparse seek grids the wallpaper runtime apparently can't satisfy when grabbing the unlock-fade still frame (B14/B15). Verified post-transcode via ffprobe `r_frame_rate == "240/1"`
...
B15|2026-05-18|V44 fix landed (`start_time=0.000000` confirmed via ffprobe) but unlock-gray still happened. Remaining diff vs Apple: `r_frame_rate` 30/1 vs 240/1, `has_b_frames` 0 vs 4, HEVC level 5.0 vs 5.2.1. The `4KSDR240FPS` directory name turns out to be a content-format contract, not just a label. Hypothesis: wallpaper runtime samples the unlock-fade still at fine intervals (multiples of 1/240s = 4.17ms) that our 30fps timeline can't satisfy. Fix: V45 — force output to 240fps via `-r 240`. ffmpeg duplicates frames from lower-fps sources. ~5× transcode time, modest file-size growth since duplicates encode cheaply.|V45
```
