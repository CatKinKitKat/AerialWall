...
V50: encode at **2880×1620**, not 4K. The wallpaper reader uses two selection levels: level 2 (lock/unlock — needs `temporal_id ≥ 1`) and level 3 (desktop apply — needs `temporal_id ≥ 3`). VT public API (HW + SW) caps HEVC Main 10 hierarchical depth at **3 sub-layers (max temporal_id 2) when output ≥ 4K** — verified across HW/SW encoders, bitrates 20–80 Mbps, and source fps 30/60/120. At ≤ 2880×1620 with `BaseLayerFrameRate = srcFps/8`, VT produces 4 sub-layers (max temporal_id 3), satisfying level 3. 2880×1620 exceeds MacBook Air M1 native (2560×1600) and 1440p Retina; upscale on 4K displays is imperceptible for video content. Apple stock 4K aerials reach 5 layers via a private encoder we can't access.
...
B19|2026-05-19|After V48 fix, custom wallpapers shipped working (lock screen + post-unlock = video). But applying a wallpaper showed **solid black desktop** until the user did a manual lock+unlock cycle (mirror image of B14, opposite side of the same axis). Live log probe revealed the wallpaper reader uses TWO selection levels: `our level is 2` for lock/unlock (needs temporal_id ≥ 1, V48 satisfied) and `our level is 3` for desktop apply (needs temporal_id ≥ 3, V48 did NOT satisfy — V48 only produced max temporal_id 1). Exhaustive VT probing: `BaseLayerFrameRate = srcFps/8` produces 4 sub-layers (max temporal_id 3) at ≤2880×1620 but caps at 3 sub-layers (max temporal_id 2) at 4K, regardless of bitrate / fps / HW vs SW encoder. Public VT cannot match Apple's stock 5-layer 4K aerials — Apple uses a private encoder. Fix: V50 — encode at 2880×1620 instead of 4K. Above MacBook Air M1 native res; imperceptible upscale on 4K displays.|V50
```
