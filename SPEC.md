...
V48: TranscodeEngine is **pure native** — `AVURLAsset` + `AVVideoComposition.Configuration` (scale/pad/color/orientation; macOS 26 value-type API) + `AVAssetWriter` w/ VideoToolbox HEVC encoder driven by `kVTCompressionPropertyKey_BaseLayerFrameRate = srcFps/2`. Produces 2-layer hierarchical HEVC (TRAIL_R at `temporal_id 0` + TSA_N at `temporal_id 1`) — sufficient for WallpaperAerialsExtension's `video-sample-reader` to find frames at non-zero `temporal_id` and not skip the entire stream (B17). ⊥ ffmpeg in runtime pipeline (only used in tests for testsrc synthesis). Side-effect: WebM container inputs fail at the `loadTracks` step (AVAssetReader doesn't support WebM on Tahoe) — surfaced as `TranscodeError.inputFormatUnsupported` with actionable UI text. Test `transcodeProducesTemporalSubLayers` verifies output via ffprobe `trace_headers` (`TSA_N` + `temporal_id > 0`)
V49: leading-black-frame trim. WallpaperAerialsExtension snapshots PTS=0 of the encoded `.mov` for the desktop wallpaper preview; user content commonly fades from black & the snapshot is black → desktop renders solid black on apply, only resolves after lock+unlock causes a re-snapshot from current playback time (B18). Mitigated by sampling source frames at 0.25s intervals up to 5s, computing Rec.709 luma, returning the first frame above mean luminance ~25/255. `AVAssetReader.timeRange` starts at that offset; `writer.startSession(atSourceTime:)` rebases output PTS to 0
...
B18|2026-05-19|After V48 fix, applying a custom wallpaper showed black on the desktop. Lock/unlock cycle resolved it (opposite of the B14 pattern). Frame extraction confirmed: source video starts with ~0.5s fade-from-black; PTS=0 of the encoded output is near-solid black (PNG≈54KB at 4K vs 13MB for content frames). WallpaperAerialsExtension snapshots PTS=0 for the desktop preview cache → black. Lock/unlock re-snapshots from current playback time → real content. Fix: V49 — scan + trim leading black frames during transcode.|V49
```
