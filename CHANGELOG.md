...
- **Logs view no longer freezes the app.** `OSLogStore.getEntries` is
  ...
  never gets a million-row payload. Default range shortened to 15 min. The 24-hour option was removed.
...
- **Structured logging**: `os.Logger` under subsystem `com.aerialwall.kit`. Inspect live via Console.app (V51)
